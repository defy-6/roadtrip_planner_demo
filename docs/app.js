const isShareMode = Boolean(window.__ROADTRIP_SHARE_MODE__);
const shareData = window.__ROADTRIP_SHARE_DATA__ || null;
const shareAssetPath = isShareMode ? './' : '/';
...
const clockToMinute = time => { const [hour, minute] = (time || '00:00').split(':').map(Number); return hour * 60 + minute; };
const minuteToClock = minute => `${String(Math.floor(minute / 60)).padStart(2, '0')}:${String(minute % 60).padStart(2, '0')}`;
...
function fitSelectionWithDayContext(selectionBounds, maxZoom = 12) {
  ...
  map.fitBounds(combined, { padding: [42, 42], maxZoom });
}
...
const geocode = (address, keyword = '') => {
  ...
  const request = geocodeQueue.then(async () => {
    const r = await fetch(`/api/geocode?address=${encodeURIComponent(address)}&keyword=${encodeURIComponent(queryKeyword)}`);
    const d = await r.json();
    await pause(700);
    if (!r.ok || !d.geocodes?.length) throw new Error(d.error || `未找到：${address}`);
    return d.geocodes[0];
  });
...
function drawMapExportLegend(context, scale) {
  ...
  rows.push({ label: '地点（按地点类型着色）', color: markerColors.spot }, { label: '航班（浅黄色弧线）', color: markerColors.flight });
  ...
  rows.forEach((row, index) => {
    ...
    context.strokeStyle = row.color; context.lineWidth = (index < dates.length ? 3.2 : 5) * scale; context.lineCap = 'round';
