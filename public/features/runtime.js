...
function confirmNewPlace(initial = {}) {
  ...
  $('#newPlaceIntro').value = ''; $('#newPlaceOpenTime').value = ''; $('#newPlaceRating').value = ''; $('#newPlaceReferenceCost').value = ''; $('#newPlaceTicketPrice').value = ''; $('#newPlaceTags').value = '';
  ...
  if (initial.name && initial.address) requestAnimationFrame(() => { queryNewPlaceLocation(); queryNewPlacePhotos(); });
...
document.head.append(Object.assign(document.createElement('style'), { textContent: '.selected-map-route{filter:drop-shadow(0 0 2px #fff) drop-shadow(0 2px 4px #173c3270)}.selected-map-point{filter:drop-shadow(0 0 3px #fff) drop-shadow(0 2px 5px #173c3290)}.selected-place-photo-marker{background:transparent!important;border:0!important;pointer-events:none}.selected-place-photo-marker img{display:block;width:42px;height:42px;object-fit:cover;border:3px solid #fff;border-radius:50%;box-shadow:0 2px 8px #173c3280,0 0 0 2px var(--place-color,#1d5b46)}' }));
  ...
  if (place.photo) {
    L.marker([lat, lng], {
      icon: L.divIcon({
        ...
        iconSize: [48, 48],
        iconAnchor: [24, 52],
        html: `<img src="${escapeHtml(place.photo)}" alt="${escapeHtml(place.name || '地点')}图片" style="--place-color:${placeTypeColor(place.type)}">`
      }),
      interactive: false,
      keyboard: false,
      zIndexOffset: 900
...
function renderLocations() {
  ...
  places.querySelectorAll('.place-card').forEach(card => {
    ...
    $('.place-photo-amap', card).onclick = async event => {
      ...
      try {
        ...
        save(); renderLocations();
      } catch (error) { alert(error.message || '高德图片查询失败。'); button.disabled = false; button.textContent = '查询高德图片'; }
    };
    card.querySelectorAll('.place-photo-candidate').forEach(button => {
      button.onclick = () => {
        const place = state.locations.find(item => item.id === card.dataset.placeId);
        const candidate = place?.photoCandidates?.[Number(button.dataset.photoIndex)]; if (!place || !candidate) return;
        place.photo = candidate.url; place.photoSource = `高德 POI · ${candidate.poiName || place.name}`; delete place.photoCandidates;
        save(); renderLocations();
      };
    });
...
function cancelPlaceConfirmation() {
  placeEditor.close();
  editingPlaceId = null; $('#placeEditorDelete').hidden = true;
...
document.head.append(Object.assign(document.createElement('style'), { textContent: '.place-detail-fields{display:grid;gap:8px;margin:0;padding:10px;border:1px solid #d9e5dc;border-radius:8px}.place-detail-fields legend{padding:0 5px;color:#315540;font-weight:700;font-size:12px}.place-detail-fields .editor-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px}.place-detail-fields input,.place-detail-fields textarea{width:100%;box-sizing:border-box}@media(max-width:560px){.place-detail-fields .editor-grid{grid-template-columns:1fr}}' }));
document.head.append(Object.assign(document.createElement('style'), { textContent: '.place-card-summary{display:grid!important;gap:7px!important;min-height:0!important;padding:9px!important;cursor:pointer}.place-card-summary:hover{border-color:#7ca890;box-shadow:0 3px 10px #173c3214}.place-card-summary-title{display:flex;align-items:center;gap:6px;min-width:0}.place-card-summary-title b{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.place-card-summary .place-photo-preview{height:104px!important;margin:0}.place-card-summary .place-photo-placeholder{height:72px}.place-card-summary.selection-mode{background:#f7fbf8}.place-card-summary .place-select{flex:0 0 auto}@media(min-width:981px){.locations-panel>.places{grid-template-columns:repeat(2,minmax(0,1fr))!important}.panel-expanded .locations-panel>.places{grid-template-columns:repeat(4,minmax(0,1fr))!important}}' }));
document.head.append(Object.assign(document.createElement('style'), { textContent: '.place-card-summary .place-photo-preview,.place-card-summary .place-photo-placeholder{width:100%!important;height:auto!important;aspect-ratio:16 / 9;box-sizing:border-box}.place-card-summary .place-photo-preview{object-fit:cover}.place-card-summary .place-photo-placeholder{min-height:0}@media(min-width:981px){.locations-panel>.places{grid-template-columns:repeat(2,minmax(0,1fr))!important}.panel-expanded .locations-panel>.places{grid-template-columns:repeat(4,minmax(0,1fr))!important}}@media(max-width:560px){.locations-panel>.places{grid-template-columns:1fr!important}}' }));
document.head.append(Object.assign(document.createElement('style'), { textContent: '.locations-panel:not(.is-expanded) .place-card.place-card-summary{display:grid!important;grid-template-columns:1fr!important}.locations-panel:not(.is-expanded) .place-card.place-card-summary>.place-card-summary-title,.locations-panel:not(.is-expanded) .place-card.place-card-summary>.place-photo-preview,.locations-panel:not(.is-expanded) .place-card.place-card-summary>.place-photo-placeholder{grid-column:1 / -1!important}.locations-panel:not(.is-expanded) .place-card.place-card-summary>.place-photo-preview{width:100%!important;height:auto!important;aspect-ratio:16 / 9!important;object-fit:cover!important}.locations-panel:not(.is-expanded) .place-card.place-card-summary>.place-photo-placeholder{display:grid!important;width:100%!important;height:auto!important;aspect-ratio:16 / 9!important}@media(min-width:981px){.locations-panel>.places{grid-template-columns:repeat(2,minmax(0,1fr))!important}}' }));
...
$('#placeEditorDelete').onclick = () => {
  const place = state.locations.find(item => item.id === editingPlaceId);
  if (!place || !confirm(`确定删除地点“${place.name || '未命名地点'}”吗？相关行程和路线关联会解除。`)) return;
  const deletedId = place.id; cancelPlaceConfirmation(); removeLocations(new Set([deletedId]));
};
...
async function queryNewPlaceLocation() {
  const name = $('#newPlaceName').value.trim(), address = $('#newPlaceAddress').value.trim();
  if (!name || !address) { alert('请先填写地点名称和完整地址。'); return; }
  const button = $('#queryNewPlaceLocation'); button.disabled = true; button.textContent = '查询中…';
  $('#newPlaceAmapStatus').textContent = '正在查询高德位置…';
  try {
    const point = await geocode(address, name);
...
$('#queryNewPlaceDetails').onclick = async event => {
  const name = $('#newPlaceName').value.trim(), address = $('#newPlaceAddress').value.trim();
  if (!name || !address) { alert('请先填写地点名称和完整地址。'); return; }
...
document.head.append(Object.assign(document.createElement('style'), { textContent: '@media(max-width:980px){html,body{height:auto!important;overflow:auto!important}main{display:block!important;min-height:100dvh;height:auto!important;max-width:none!important;margin:0!important;padding:10px 12px 28px!important}.app-header{display:grid!important;grid-template-columns:minmax(0,1fr) auto!important;gap:10px!important;align-items:start!important}.app-header .brand-lockup{grid-column:1!important;grid-row:1!important;min-height:34px}.app-header h1{font-size:25px!important}.app-header .top-actions{grid-column:2!important;grid-row:1!important;display:flex!important;justify-content:flex-end!important;align-items:flex-start!important;max-width:72vw;gap:5px!important}.app-header .plan-controls{padding:2px!important;border:0!important;background:transparent!important}.app-header .plan-controls select{min-width:132px!important;height:32px!important;border:1px solid #d8e4dc!important;border-radius:7px!important;background:#fffdf8!important}.app-header .plan-actions{display:none!important}.app-header .header-utilities{display:none!important}.app-header .hero{grid-column:1/-1!important;grid-row:2!important;width:100%!important;height:auto!important;min-height:0!important;margin:0!important;padding:10px 12px!important;display:grid!important;grid-template-columns:minmax(0,1fr) auto!important;align-items:center!important;gap:8px!important;border-radius:10px!important}.hero>div:first-child{display:block!important;min-width:0}.hero #tripName{width:100%!important;max-width:none!important;font-size:15px!important}.hero .route-stat{width:auto!important;justify-content:end!important;gap:6px!important}.route-metric{display:none!important}.route-summary-btn{padding:6px 8px!important;font-size:10px!important}.hero .route-summary-detail{position:static!important;grid-column:1/-1!important;width:auto!important;max-height:210px!important;margin:0!important;font-size:11px!important}.schedule-panel,.content,.locations-panel{height:auto!important;min-height:0!important;margin-top:12px!important;padding:12px!important;border-radius:12px!important}.schedule-panel>.aside-head{align-items:flex-start!important;gap:8px!important}.schedule-panel>.aside-head>div:first-child{padding-top:4px}.schedule-panel>.aside-head h2{font-size:16px!important}.schedule-panel>.aside-head>div:last-child{justify-content:flex-end!important;flex-wrap:wrap!important;overflow:visible!important;gap:5px!important}.schedule-panel>.aside-head button,.schedule-panel>.aside-head select{min-height:32px!important;padding:6px 8px!important;font-size:10px!important}.schedule-panel #scheduleHint{display:none!important}.schedule{margin-top:10px!important}.schedule-scroll{max-height:min(62dvh,560px)!important;border-radius:9px!important;overscroll-behavior:contain;touch-action:pan-x pan-y}.calendar-grid{--grid-width:max-content!important;grid-template-columns:52px repeat(var(--days),minmax(166px,1fr))!important}.calendar-grid .calendar-head{padding:7px 8px!important}.calendar-grid .calendar-head b{font-size:13px!important}.calendar-grid .calendar-head small{font-size:10px!important}.time-rail,.calendar-day{height:1008px!important}.content>.map-panel{height:auto!important;min-height:0!important;display:grid!important;grid-template-rows:minmax(330px,56dvh) auto auto!important}.content>.map-panel .map{height:auto!important;min-height:330px!important}.content>.map-panel .route-detail{min-height:128px!important;max-height:230px!important;margin:8px 0 0!important;padding:12px!important}.map-route-legend{left:8px!important;bottom:8px!important;max-width:calc(100% - 16px)!important;max-height:130px!important;font-size:9px!important}.map-export-control{left:8px!important;top:56px!important}.locations-panel{height:560px!important}.locations-panel>.aside-head{align-items:center!important}.locations-panel>.places{padding-right:4px!important}.location-toolbar{gap:5px!important}.location-toolbar input{min-width:0!important;flex:1 1 100%!important}.location-toolbar select{flex:1 1 42%!important}.location-toolbar button{flex:0 0 auto!important}.event-editor{width:100vw!important;max-width:none!important;max-height:100dvh!important;margin:0!important;border-radius:0!important}.editor-form{padding:14px 14px calc(14px + env(safe-area-inset-bottom))!important}.editor-form h3{top:-14px!important;margin:-14px -14px 2px!important;padding:14px!important}.editor-grid{grid-template-columns:1fr!important}.editor-actions{bottom:calc(-14px - env(safe-area-inset-bottom))!important;margin:2px -14px calc(-14px - env(safe-area-inset-bottom))!important;padding:10px 14px calc(10px + env(safe-area-inset-bottom))!important}.route-inline-grid{grid-template-columns:1fr!important}}@media(max-width:420px){main{padding-left:8px!important;padding-right:8px!important}.app-header .top-actions{max-width:65vw}.app-header .plan-controls select{min-width:116px!important;max-width:156px!important}.schedule-panel,.content,.locations-panel{padding:10px!important}.schedule-panel>.aside-head{display:block!important}.schedule-panel>.aside-head>div:last-child{justify-content:flex-start!important;margin-top:8px}.calendar-grid{grid-template-columns:48px repeat(var(--days),minmax(154px,1fr))!important}.content>.map-panel{grid-template-rows:minmax(300px,53dvh) auto auto!important}.content>.map-panel .map{min-height:300px!important}.route-detail .detail-title{font-size:14px!important}.route-detail .detail-key{font-size:11px!important}.locations-panel{height:520px!important}}' }));
